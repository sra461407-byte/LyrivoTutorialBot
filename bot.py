...
def get_main_keyboard():
    buttons = [
        [
            InlineKeyboardButton(
                text=f"{data['name']} - {data['price']}", callback_data=key
            )
        ]
...
async def handle_screenshot(message: Message):
    ...
    if user_id not in user_orders:
        await message.answer(
            "እባክዎን አስቀድመው መግዛት የሚፈልጉትን ምርት ለመምረጥ /start የሚለውን ይጫኑ።"
        )
        return
    ...
    admin_kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✅ Approve", callback_data=f"app_{user_id}_{prod_key}"
                ),
                InlineKeyboardButton(
                    text="❌ Reject", callback_data=f"rej_{user_id}_{prod_key}"
                ),
            ]
        ]
    )
    ...
    await bot.send_photo(
        chat_id=ADMIN_ID, photo=photo_id, caption=admin_text, reply_markup=admin_kb
    )

    await message.answer(
        "የከፈሉበት ደረሰኝ ለአድሚን ተልኳል። ክፍያው ተራጋግጦ አድሚን እንዳጸደቀው ቦቱ ምርቱን ወዲያውኑ ይልክልዎታል!"
    )
